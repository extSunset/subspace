...
use crate::commands::benchmark::shared::{PlotMetadataHeader, RESERVED_PLOT_METADATA};
...
use subspace_core_primitives::{SectorIndex, SectorId, PieceOffset, Record};
use subspace_farmer::single_disk_farm::SingleDiskFarmInfo;
...
pub(crate) fn benchmark_reading(benchmark_args: BenchmarkArgs) -> Result<()> {
    let BenchmarkArgs {
        base_path,
        mut sector_count,
        sample_size,
    ...
    info!("Initializing piece reading benchmark.");
   ...
   if !base_path.exists() {
        error!("Base path does not exist. base_path = {}", base_path.display());
        return Err(anyhow!("Base path does not exist. base_path = {}", base_path.display()));
    }

    if !base_path.is_dir() {
	error!("Base path is not a directory. base_path = {}", base_path.display());
        return Err(anyhow!("Base path is not a directory. base_path = {}", base_path.display()));
    }

    debug!("Base path exists and is valid.");

    debug!("Loading single disk farm info.");
    let single_disk_farm_info = match SingleDiskFarmInfo::load_from(&base_path).context("Failed to load SingleDiskFarmInfo.").unwrap() {
        Some(i) => i,
        None => {
            return Err(anyhow!("SingleDiskFarmInfo is empty. base_path = {}", base_path.display()));
        },
    };
    ...
    let plot_file = OpenOptions::new()
        ...
        .create(false)
        .open(&plot_file_path)
        .context(format!("Failed to open plot file. path = {}", plot_file_path.display()))?;
    
    let target_sector_count = plot_file.metadata().context("Failed to retrieve plot file metadata.")?.len() / sector_size as u64;
    let mut sectors_metadata = Vec::<SectorMetadataChecksummed>::with_capacity(target_sector_count as usize);

    let metadata_file_path = base_path.join("metadata.bin");
    debug!("Opening a metadata file at path {}", metadata_file_path.display());
    let metadata_file = OpenOptions::new()
        .read(true)
        .write(true)
        .create(false)
        .open(&metadata_file_path)
        .context(format!("Failed to open metadata file. path = {}", metadata_file_path.display()))?;

    debug!("Memory mapping of plot.");
    let plot_mmap = unsafe {
        Mmap::map(&plot_file).context("Failed to create memory mapping for plot file.")?
    };
	...
	debug!("Advicing random IO for plot and metadata memory mappings.");
        plot_mmap.advise(memmap2::Advice::Random).context("Failed to set advice for plot mmap")?;
        metadata_mmap.advise(memmap2::Advice::Random).context("Failed to set advice for metadata mmap")?;
    }
    ...
    info!("Running piece reading benchmark with single sector.");
    ...
    group.bench_function("piece/memory", |b| {
	    ...
	    read_piece::<PosTable>(
                ...
                black_box(&(SectorId::new(public_key.hash(), 0 as SectorIndex))),
    ...
    info!("Running piece reading benchmark. sector_count = {}", sector_count);
    ...
    group.bench_function("piece/disk", move |b| {
        b.iter_custom(|iters| {
		    ...
		    read_piece::<PosTable>(
			black_box(piece_offset),
			black_box(&(SectorId::new(public_key.hash(), sector_index as SectorIndex))),

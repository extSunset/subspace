...
use sp_domains::{DomainGenesisStorage, DomainId, DomainsApi, DomainsDigestItem};
use sp_runtime::traits::Block as BlockT;
use std::marker::PhantomData;
use std::sync::Arc;

pub struct BootstrapResult<CBlock: BlockT> {
    // The `DomainGenesisStorage` used by the domain instance starter to
    // construct the genesis state of the domain instance
    pub domain_genesis_storage: DomainGenesisStorage,
    ...
    CClient::Api: DomainsApi<CBlock, NumberFor<Block>, Block::Hash>,
{
    ...
    pub async fn fetch_domain_bootstrap_info(
        ...
        if let Some((domain_genesis_storage, domain_created_at)) = self
            .consensus_client
            .runtime_api()
            .domain_genesis_storage(best_hash, self_domain_id)?
        {
        ...
        let (domain_genesis_storage, domain_created_at) = 'outer: loop {
                    ...
                    if let Some(domain_id) = item.as_domain_instantiation() {
                        if domain_id == self_domain_id {
                            let res = self
                                .consensus_client
                                .runtime_api()
                                .domain_genesis_storage(header.hash(), domain_id)?;
